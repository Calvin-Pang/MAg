...
def searchFile(key,startPath = '.'):
    '''
    This is used to search files containg the key name from a folder.
    '''
...
def convert_dataset(json_path,num_features):
    '''
    This is used to convert the json file containing features into the X,y list which can be used directly in SVM.
    '''
    dataset_dict = json_file_to_dict(json_path)
    num_samples = len(dataset_dict)
    X = [[0]*num_features for i in range(num_samples)]
    y = []
    count = 0
