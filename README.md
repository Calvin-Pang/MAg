...
**Update: Our paper has been published: https://ieeexplore.ieee.org/document/9761500.**

Here is the video of our presentation prepared for IEEE ISBI 2022: https://youtu.be/Ln60y3aGthc
...
The comparison of our method and the two commonly used methods (counting and averaging) is shown below:
...
The proposed method is shown in the figure below:
...
2. We have split the CRC_DX and STAD datasets into training sets, validation sets, and testing sets at the patient level. So, after downloading them from the link, please split the dataset according to the patient name we list in the [**name_patient**](https://github.com/Calvin-Pang/MAg/tree/main/name_patient) file. 

3. Certainly, if you want to change the way of splitting the data set, you can also split the data set by yourself. For your reference, you can use the code in the link https://github.com/jnkather/MSIfromHE/blob/master/step_05_split_train_test.m to do this split.
...
## How to use MAg?
Our method code is in the [**demo**](https://github.com/Calvin-Pang/MAg/tree/main/demo) file. Follow the steps below, and you can easily use MAg to complete training and prediction.
...
Also, here are some very helpful links that teaches you how to use Timm: https://fastai.github.io/timmdocs/ and https://rwightman.github.io/pytorch-image-models/

2. Secondly, after using the above process to obtain the classification model at the patch level, you can use [**2.0.patch2image_counting.ipynb**](https://github.com/Calvin-Pang/MAg/blob/main/demo/2.0%20patch2patient_counting.ipynb) to make the patch-level prediction. In this process, just follow the operation of the code in the notebook, and you can get patch-level probabilities and histogram-based features. 


3. After getting the patch-level model and patient-level histogram-based features from the processes above, you can now train the patient-level classification models. Here, we provide two different methods to complete this training, which means you can train it in an SVM with [**2.1.patient-level MAg-SVM_histogram.ipynb**](https://github.com/Calvin-Pang/MAg/blob/main/demo/2.1.patient-level%20MAg-SVM_histogram.ipynb) or in a two-layer fully-connected neural network with [**2.2.patient-level MAg-network.ipynb**](https://github.com/Calvin-Pang/MAg/blob/main/demo/2.2.patient-level%20MAg-network.ipynb). If your dataset is not very large, we suggest you are using the **2.1** while you had better use **2.2** if you have a huge dataset(e.g., a dataset which contains 100000 patients).
...
**NOTE**: the patient-level training also requires you to follow the split you did before, so please remember to save the patient-level histogram-based features in xlsx files like train.xlsx, validation.xlsx, and test.xlsx
...
As you can see, these seemingly complex and illogical Jupyter notebooks do not achieve the modularity and portability of MAg. So, we provide a very early version of the MAg_lib library and hope it can help you call it directly (Up to now, we only provide the MAg method using SVM. In the future, we may add other ML methods into it). Here are some instructions and tips that may help you when using the MAg_lib.

1. In the MAg_lib, in order to achieve a more concise code, we no longer use **xlsx** format files to store data. Instead, we use the **dict(or JSON)** format to perform the functions. So in ```MAg_lib.MAg.convert_format```, we provide the functions **json_file_to_dict** and **dict_to_json_file** to do the conversion task between **dict** and **json** file.
...
First, you need three JSON files that associate the sample names with the paths of the patients, corresponding to the training set, validation set, and test set. The format of the files is like:
...
Second, you need to do the patch-level prediction with the classification, which is the same as step 1 in [**How to use MAg?**](#how-to-use-mag). With the function in MAg_lib, you can directly get the dict containing patient_level features (**Please remember to do this step in all three sets so that you can do the next step!**). Here is the example code:
...
```X,y```represent training set and ```X_val,y_val``` represent validation set. The next three parameters are three lists that provide the kernels, penalty coefficients, and class weights you want to let this function try.
...
The experiments were performed on a Google Colab workstation with an NVIDIA Tesla P100 GPU. In stage I, five prevalent approaches have been used to be the baseline feature extractors, including ResNet, MobileNetV2, EfficientNet, Dpn, and ResNext models. In stage II, we mainly use SVM to complete it. Moreover, to assess the generalizability, the experiments above were done in both the CRC dataset and the STAD dataset.
Below are the results of our experiments and comparison between MAg and two commonly used methods (counting and averaging):
